...
{
    public partial class JuraForestForm3 : Form
    {
        private readonly LocItems _items;

        int[,] obstacles = new int[27, 4];
        PictureBox[] obstacle = new PictureBox[27];
        ...
        private void TeleportChecker(int[] newPosition, int[,] moblocation, string[] mob)
        {
            for (int i = 0; i < moblocation.GetLength(0); i++)
            {
                   ...
                   (newPosition[2] >= moblocation[i, 2] && newPosition[0] <= moblocation[i, 2])))
                {
                    ...
                    {
                        Program.items.location[0] = 770;
                        Program.items.location[1] = 110;
        ...
        private void JuraForestForm3_Load(object sender, EventArgs e)
        {
            item[0] = item1;
            item[1] = item2;
            item[2] = item3;
        ...
        private void JuraForestForm3_KeyDown_1(object sender, KeyEventArgs e)
        {
