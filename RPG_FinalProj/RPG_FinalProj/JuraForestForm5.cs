...
{
    public partial class JuraForestForm5 : Form
    {
        private readonly LocItems _items;

        int[,] obstacles = new int[32, 4];
        PictureBox[] obstacle = new PictureBox[32];
        ...
        private void TeleportChecker(int[] newPosition, int[,] moblocation, string[] mob)
        {
            for (int i = 0; i < moblocation.GetLength(0); i++)
            {
                    ...
                    (newPosition[2] >= moblocation[i, 2] && newPosition[0] <= moblocation[i, 2])))
                {
                    ...
                    {
                        Program.items.location[0] = 820;
                        Program.items.location[1] = 845;
                   ...
                   (newPosition[3] >= moblocation[i, 3] && newPosition[1] <= moblocation[i, 3])))
                {
                    if (mob[i] == "TeleportLeft")
                    {
                        ...
                        Program.items.location[0] = 1250;
                        Program.items.location[1] = 475;
                        JuraForestCave JFF = new JuraForestCave();
                        this.Hide();
                        JFF.ShowDialog();
                        this.Close();
                    }
        ...
        private void JuraForestForm5_Load(object sender, EventArgs e)
        {
            item[0] = item1;
            item[1] = item2;
            item[2] = item3;
        ...
        private void JuraForestForm5_KeyDown_1(object sender, KeyEventArgs e)
        {
